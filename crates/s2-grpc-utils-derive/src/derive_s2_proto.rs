            ...
            if let Some(map_fn) = f.map_fn.as_ref() {
              ...
              let value_expr = if f.proto_enum {
                ...
                setter_lines.push(quote! {
                  packed.#seter_ident(
                    <#field_ty as s2_grpc_utils::S2ProtoEnum<_>>::into_proto_enum(value.#field_ident)
                  );
                });
            ...
            let field_expr = if let Some(map_fn) = f.map_fn.as_ref() {
              ...
              if f.proto_enum {
                getter_lines.push(quote! {
                  let #field_ident = <#field_ty as s2_grpc_utils::S2ProtoEnum<_>>::unpack_enum(value.#value_field_ident());
                });
                quote! {
                  #field_ident
                }
